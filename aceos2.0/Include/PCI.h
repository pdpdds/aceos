...
#define __PCI_MaxClass 0x12 //should be removed
...
struct PCIConfigurationSpace
	...
	union
		...
		struct
			...
			} __attribute__ ((packed)) NonBridge;
	     	struct
			{
			DWORD BaseAddress0;
			DWORD BaseAddress1;
		...
		struct
			...
			}__attribute__ ((packed)) CardBus;
		}__attribute__ ((packed)) PreDefinedHeader;
	};
...
struct PCIDeviceDetails
	...
	BYTE Device;
	BYTE Function;
	struct PCIConfigurationSpace PCIConfDetails;
	};
