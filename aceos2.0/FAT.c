/*
    ...
    Date: 				09-Sep-2002 22:42
    Modified on 03-Jun-2003 22:49
        Replaced Collection with List
        standard C Library is used instead of many functions such malloc, strcpy etc
    Modified on 24-Oct-2003 12:54
        Changes made to accomdate the changes in GFS
    Modified on 25-Aug-2005 
        split long functions into smaller ones
    
    TODO - 
                * make the FAT calculations earlier and store it 
...
void FAT_Initialize(LPFAT lpFATDetails, LPCTSTR szDevicePath, LPPARTITION lpPart, BYTE * pFirstSector)
{
    ...
    strcpy(lpFATDetails->szDevicePath, szDevicePath );             //copy the Device Path
...
*/
UINT32 FAT_GetLogicalSector(LPFAT lpFAT, UINT32 dwCluster,UINT16 dwSector)
{
...
UINT32 FAT_GetClusterCluster(LPFAT lpFAT, UINT32 dwLogicalSector)
{
    UINT32 dwResult;
...
UINT32 FAT_FATSectorNumber(LPFAT lpFAT, UINT32 dwCluster)
{
    UINT32 FATOffset;
...
UINT32 FAT_FATSectorOffset(LPFAT lpFAT, UINT32 dwCluster )
{
    UINT32 dwFATOffset;
    if ( lpFAT->FDI.bFATType == 12 )
...
UINT32 FAT_FirstSectorOfCluster(LPFAT lpFATDetails, UINT32 Cluster)
{
...
UINT16 FAT_RootDirStartSector(LPFAT lpFAT)
{
...
UINT16 FAT_RootDirStartCluster(LPFAT lpFAT)
{
    if ( lpFAT->FDI.bFATType == 12 || lpFAT->FDI.bFATType == 16 )
...
UINT32 FAT_GetFATEntry(LPFAT lpFAT, UINT32 dwCluster , BYTE * pSector )
{
...
HANDLE FAT_CreateFile(LPFSHANDLE lpFS, LPCTSTR lpFilePath, DWORD dwOpenMode, DWORD dwShareMode, DWORD dwCreationDisposition, DWORD dwFlags )
{
    ...
    else        //open existing file
    {
        ...
        ALLOC_AND_CHECK(lpFileInfo, LPFATOPENEDFILEINFO, sizeof(FATOPENEDFILEINFO), 0);
        ...
        ALLOC_AND_CHECK(lpFileInfo->lpBuffer, BYTE *, lpFileInfo->lpFATInfo->FDI._BPB.bSectorsPerCluster * lpFileInfo->lpFATInfo->FDI._BPB.wBytesPerSector, 0);
...
UINT32 MoveFilePointerToNextCluster(LPFATOPENEDFILEINFO lpFileInfo, BOOLEAN bFillBuffer )
{
    ...
    if  ( !FAT_IsEndOfClusterChain(lpFATInfo,lpFileInfo->dwCurrentCluster)  )
    {
        ...
        if ( bFillBuffer )
        {
            ReadGSHDevice(lpFATInfo->szDevicePath , PART_BEGINING(&lpFATInfo->Part) + FAT_FirstSectorOfCluster(lpFATInfo,lpFileInfo->dwCurrentCluster), lpFATInfo->FDI._BPB.bSectorsPerCluster, (BYTE *)lpFileInfo->lpBuffer);
            if ( GetLastError() )
            {
        ...
        {
            ReadGSHDevice(lpFATInfo->szDevicePath, dwFATSector, 2, (BYTE *)lpFileInfo->lpSectorBuffer);
            if ( GetLastError() )
            {
...
UINT32 DeleteFileEntry(LPFSHANDLE lpFS, LPCTSTR lpFileName)
{
    ...
    DWORD dwCurrentCluster, dwLBASector, dwBufferSize;
    BYTE * lpBuffer;
    ...
    ALLOC_AND_CHECK(lpBuffer, BYTE *, dwBufferSize, 1);    
    do 
    {
        ReadGSHDevice(lpFAT->szDevicePath , dwLBASector, dwBufferSize / lpFAT->FDI._BPB.wBytesPerSector, (BYTE *)lpBuffer);
        if ( GetLastError() )
        {
        ...
        for(;dwBufferIndex<dwBufferSize;dwBufferIndex+=sizeof(DIRENTRY) )
        {
            ...
            if ( GetFileNameFromDEInfo(lpDEInfo, szDEFileName) )
            {
                if ( stricmp(szDEFileName, szFileName) == 0 )
                {
                    ...
                    WriteGSHDevice( lpFAT->szDevicePath, dwLBASector, dwBufferSize / lpFAT->FDI._BPB.wBytesPerSector, (BYTE *)lpBuffer);
                    return 0;
                }
            }
        }
        ...
        {
            ReadGSHDevice(lpFAT->szDevicePath , PART_BEGINING(&lpFAT->Part) + FAT_FATSectorNumber(lpFAT, dwCurrentCluster), 1, (BYTE *)lpBuffer);
            if ( GetLastError() )
            {
...
#define FIND_DE_IN_FOLDER() \
    ...
    { \
        ...
        {\
            ReadGSHDevice(lpFileInfo->lpFATInfo->szDevicePath, dwLBASector, lpFileInfo->lpFATInfo->FDI._BPB.bSectorsPerCluster, (BYTE *)lpFileInfo->lpBuffer);\
            if ( GetLastError() )\
            {\
...
#define FILL_FILE_INFO() \
    ...
    strncpy(szFileName, lpDEInfo->bFileName,8);\
    szFileName[8]=0;\
    strncpy(szExt, lpDEInfo->bExtension,3);\
    szExt[3]=0;\
...
HANDLE FAT_FindFirstFile(LPFSHANDLE lpFS, LPCTSTR lpFilePath, LPFILEINFO lpFINFO)
{
    ...
    ALLOC_AND_CHECK(lpDEInfo, LPDIRENTRY, sizeof(DIRENTRY), NULL);
    SplitPath(lpFilePath, szDrive, szDirectory, szFileName, szExtension);
    ...
    ALLOC_AND_CHECK( lpFileInfo, LPFATFINDFILEINFO, sizeof(FATFINDFILEINFO), 0);
    lpFileInfo->lpFATInfo = (LPFAT) lpFS->FSSpecificFields[0];
    if ( lpFileInfo->lpFATInfo == NULL )
    {
    ...
    if ( szDirectory[0] != 0 && strcmp(szDirectory,"/")!=0 && strcmp(szDirectory,"\\")!=0  )       //should traverse the directories first
    {
        ...
        ALLOC_AND_CHECK( lpFileInfo->lpBuffer, BYTE *, dwNoOfBytes, 0);
...
BYTE EnumerateFilesInFolder(LPFAT lpFATInfo, LPDIRENTRY FolderDE, BYTE (*CallBackFn)(LPDIRENTRY, void * ,LPDIRENTRY), void * CallBackPara , LPDIRENTRY lpResult)
{
    ...
    ALLOC_AND_CHECK( pDirData, UINT16 *, dwNoOfBytes, 0);
    ...
    do
    {
        ReadGSHDevice(lpFATInfo->szDevicePath , PART_BEGINING(&lpFATInfo->Part) + FAT_FirstSectorOfCluster(lpFATInfo,dwDirCluster), lpFATInfo->FDI._BPB.bSectorsPerCluster, (BYTE *)pDirData);
        if ( GetLastError() )
            return 0;
        ...
        ReadGSHDevice(lpFATInfo->szDevicePath , PART_BEGINING(&lpFATInfo->Part) + FAT_FATSectorNumber(lpFATInfo,dwDirCluster), 1, (BYTE *)pDirData);
        if ( GetLastError() )
            return 0;
...
BYTE EnumerateFilesInRoot(LPFAT lpFATInfo, BYTE (*CallBackFn)(LPDIRENTRY ,void *,LPDIRENTRY), void * CallBackPara, LPDIRENTRY lpResult )
{
    ...
    ALLOC_AND_CHECK( pRDSector, UINT16 *, dwNoOfBytes, 0);
    ...
    {
        ReadGSHDevice(lpFATInfo->szDevicePath , dwLBASec, 1, (BYTE *)pRDSector);
        if ( GetLastError() )
        {
...
int FAT_Init()
{
    ...
    LPFSHANDLE lpNewFS;
    ALLOC_AND_CHECK( lpNewFS, LPFSHANDLE, sizeof(FSHANDLE), 0);
...
void CheckAndInstallFAT(LPCTSTR szDevicePath, LPPARTITION lpPart)
{
        ...
        PART_TYPE(lpPart)==0x11 || PART_TYPE(lpPart)==0x14 || PART_TYPE(lpPart)==0x1B || PART_TYPE(lpPart)==0x1C || PART_TYPE(lpPart)==0x1E ) 
    {
        LPFSHANDLE lpNewFS;
        LPFAT lpNewFAT;
        ALLOC_AND_CHECK( lpNewFS, LPFSHANDLE, sizeof(FSHANDLE),  );
        ALLOC_AND_CHECK( lpNewFAT, LPFAT, sizeof(FAT),  );
