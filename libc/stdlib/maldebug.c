...
#warning ******check the below comment lines
...
bool __malloc_check_header(__maldbg_header_t *header)
{
    ...
    {
        printf(L"%p: memory check error (1): got 0x%x, should be 0xdeadbeef\n",  header + 1, header->magic[0]);
        return false;
    }

    if (header->magic[1] != (header->magic[0] ^ header->size))
    {
        printf(L"%p: memory check error (2): got 0x%x, should be 0x%x\n", header + 1, header->magic[1], header->magic[0] ^ header->size);
        return false;
    }

	if (__malloc_find_block(header + 1) == NULL)
	{
		printf(L"%p: memory block not found in list\n", header + 1);
		return false;
	}
...
void __free(void *ptr, const char *file, int line)
{
    ...
    if (ptr != NULL)
    {
		...
		header->file = file;
		header->line = line;
		memset(header + 1, 0xdd, header->size - sizeof(*header));
        free(header);
    }
}
