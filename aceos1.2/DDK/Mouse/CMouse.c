...
BYTE PortNo=0, SysMousePortType=-1, Sys_MouseHardwareID=0;
//---------------------------------------------------------------------
...
const COMInt[4]={0x04,0x03,0x04,0x03}; /* How much serial ports you have is not a
...
BYTE InitMouse()
	...
	for(PortNo=0;PortNo<4 && MouseProtocol == -1;PortNo++)
		...
		InPortByte(COMIOBase[PortNo]+LSR);//clear error bits
		InPortByte(COMIOBase[PortNo]+MCR);//clear state bits
		InPortByte(COMIOBase[PortNo]);//clear receive buffer

		OutPortByte(COMIOBase[PortNo],0); //interrupts off
		OutPortByte(COMIOBase[PortNo]+1,0); //disable FIFO
		...
		do 	{
			...
			}while(T+300>GetTimerTick() && !DataAvl);
		if ( !DataAvl )
			continue;
	...
	Sys_MouseHardwareID = RegisterDeviceDriver(0, 0, 2, "Generic Mouse Driver V0.9", "Power Team", MouseHand);
	if ( Sys_MouseHardwareID )
		{DisableNMI();
		InitializeSCC(SysMouseIOPort);
		SetIDT(32+COMInt[PortNo],MouseHandler);
		OutPortByte( SysMouseIOPort+MCR, 0x0B );   	/* enable DTR,RTS and OUT2 */
		InPortByte(COMIOBase[PortNo]+LSR);//clear error bits
		InPortByte(COMIOBase[PortNo]+MCR);//clear state bits
		InPortByte(COMIOBase[PortNo]);//clear receive buffer

		OutPortByte(COMIOBase[PortNo],1); //activate data avl interrupt
		OutPortByte(COMIOBase[PortNo]+1,0); //disable FIFO

		BYTE Mask = InPortByte( PIC_STS ); 			/* Reading status of PIC */
		Mask = Mask & ( (PortNo==0||PortNo==3)?0xEF:0xF7);/* Setting the appropriate bit for COM1(11101111) and COM2(11110111) */
		OutPortByte( PIC_STS, Mask );
		EnableNMI();	
		}
	else
		Printf("\n\rMouse : Unable to register driver.");
	return Sys_MouseHardwareID;
	}
